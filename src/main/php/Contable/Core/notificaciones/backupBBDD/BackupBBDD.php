<?php
namespace Contable\Core\notificaciones\backupBBDD;

use Contable\Core\utils\XTemplate;
...
class BackupBBDD{
	...
	function backupDatabaseTables($dbHost,$dbUsername,$dbPassword,$dbName, $filename,$tables = '*' ){
	    ...
	    //save file
		$nameFile ='db_contable_' . date("Y-m-d-H-i-s") .'.sql';
	...
	public function send(){
		...
		$this->setPathBackup(ContableConfig::getInstance()->getAppPathCore()."/backup/");
		
		$filename = 'db_contable_' . date("Y-m-d-H-i-s") . '.zip';
		$backupFile = $this->getPathBackup().$filename;

		self::backupDatabaseTables(ContableConfig::getInstance()->getDbHost(),ContableConfig::getInstance()->getDbUser(),ContableConfig::getInstance()->getDbPassword(),ContableConfig::getInstance()->getDbName(),$filename);
		...
		$subjectMail = 'BackUp cose_contable';
		...
		$template->assign("titulo", "En el archivo adjunto se encuentra el BackUp de la BBDD cose_contable realizado el ".date('d/m/Y H:i:s'));
		...
		ContableUtils::sendMail(ContableConfig::TEST_MAIL_TO,ContableConfig::TEST_MAIL_TO, $subjectMail, $mensaje, $attachs);
		unlink($backupFile);
