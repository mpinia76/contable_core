...
use Contable\Core\service\ICajaService;
...
class CajaServiceImpl extends CrudService implements ICajaService {
	...
	function abrirCaja( Caja $caja, User $user ){
		...
		$transferencia->setOrigen( ContableUtils::getCuentaCajaChica() );
	...
	function cerrarCaja(Caja $caja, User $user){
		...
		try {
			...
			$transferencia->setDestino( ContableUtils::getCuentaCajaChica() );
